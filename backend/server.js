...
import bcrypt from "bcryptjs";         // ✅ use bcryptjs (no native build)
...
async function initDb() {
  ...
  await db.exec(`
    ...
    -- Optional OAuth token stores (placeholders)
...
function withdrawableFromWallet(w) {
  const cap = 0.5 * Number(w.survey_usd_total || 0); // 50% of survey total
...
// Back-compat route /api/wallet/:user_id
app.get("/api/wallet/:user_id", async (req, res) => {
  req.query.user_id = req.params.user_id;
  return app._router.handle(req, res); // re-use handler above
});
...
app.post("/api/paypal/deposit/create", authOptional, async (req, res) => {
  try {
    ...
    // Remember who is creating this order (optional store)
    res.json(data);
  } catch {
...
// Capture order and credit owner funds (not withdrawable)
app.post("/api/paypal/deposit/capture", authOptional, async (req, res) => {
  try {
    ...
    await db.run(
      "INSERT INTO ledger (user_id, type, amount, meta) VALUES (?,?,?,?)",
      user_id, "deposit_paypal", total, JSON.stringify({ order_id })
    );
...
app.get("/api/admin/user/:user_id", (req, res, next) => {
  ...
  const ledger = await db.all(
    "SELECT * FROM ledger WHERE user_id=? ORDER BY id DESC LIMIT 200",
    user_id
  );
...
/* =========================
   RADIO SEARCH (Radio-Browser directory)
========================= */
const RADIO_HOSTS = [
  ...
  for (const base of RADIO_HOSTS) {
    try {
      const { data } = await axios.get(`${base}/json/stations/search`, {
        params: { name: q, limit: 12, hidebroken: true },
        timeout: 8000,
      });
...
initDb().then(() =>
  app.listen(PORT, () =>
    console.log("EngageHubCoin running on http://localhost:" + PORT)
  )
);
